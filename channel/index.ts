...
mcp.setRequestHandler(CallToolRequestSchema, async (req) => {
  if (req.params.name === 'reply') {
    const { text, session_id } = req.params.arguments as {
      text: string
      session_id?: string
    }
  ...
  if (req.params.name === 'memory_remember') {
    const { content, project_dir } = req.params.arguments as { content: string; project_dir?: string }
  ...
  if (req.params.name === 'memory_forget') {
    const { id } = req.params.arguments as { id: number }
  ...
  req.on('end', async () => {
    try {
      ...
      if (url.pathname === '/send') {
        ...
        const { text, source = 'datawatch', session_id = '' } = JSON.parse(body) as {
          text: string
          source?: string
          session_id?: string
        }
        ...
        const { request_id, behavior } = JSON.parse(body) as {
          request_id: string
          behavior: 'allow' | 'deny'
        }
        await mcp.notification({
...
async function callParent(path: string, method: string, body?: unknown): Promise<string> {
