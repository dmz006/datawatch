...
run_t17() {
  ...
  if story_matches_filter "$tags"; then
    ...
    proj=$(api POST /api/profiles/projects "{\"name\":\"$proj_name\",\"description\":\"E2E test project\"}")
    save_evidence "TS-241" "0_create_project.json" "$proj"
    proj_id=$(echo "$proj" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('id',''))" 2>/dev/null || echo "")

    if [[ -n "$proj_id" ]]; then
      # Now create PRD with project_profile
      ...
      prd=$(api POST /api/autonomous/prds "{\"name\":\"$prd_name\",\"description\":\"Autonomous E2E test\",\"spec\":\"$spec\",\"project_profile\":\"$proj_id\",\"max_depth\":2}")
      ...
      if [[ -n "$prd_id" ]]; then
        ok "Autonomous PRD created with decomposition capability (LLM backend available)"
        add_cleanup "automaton" "$prd_id"
        add_cleanup "profile-proj" "$proj_id"
      else
