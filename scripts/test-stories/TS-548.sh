...
_story_ts_548() {
  local resp
  resp=$(api POST /api/mcp/call '{"tool":"docs_search","params":{"query":"vault secrets backend"}}')
  save_evidence TS-548 "search.json" "$resp"
  local hits
  ...
  if [[ "$hits" -gt 0 ]] 2>/dev/null; then
    if echo "$resp" | grep -qi "secrets-manager"; then
      ok "docs_search returned $hits hits including secrets-manager.md"
    else
