#!/usr/bin/env bash
# TS-368 — GET /api/autonomous/guardrail_profiles/{id} round-trip
...
_story_ts_368() {
  ...
  # GET by id
  local resp
  resp=$(api GET "/api/autonomous/guardrail_profiles/$pid")
  save_evidence TS-368 "resp.json" "$resp"
  if assert_json "$resp" '"id" in d'; then
    ok "GET /api/autonomous/guardrail_profiles/$pid round-trips successfully"
