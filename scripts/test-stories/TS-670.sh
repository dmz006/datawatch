...
_story_ts_670() {
  ...
  local cfg_code
  cfg_code=$(api_code PUT /api/config '{"vision.enabled":true,"vision.backend":"ollama","vision.endpoint":"http://localhost:11434","vision.model":"moondream:latest"}' | sed -n 's/.*__HTTP_CODE_\([0-9]*\)__.*/\1/p')
  if [[ ! "$cfg_code" =~ ^2 ]]; then
