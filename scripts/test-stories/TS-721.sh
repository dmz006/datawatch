...
_story_ts_721() {
  ...
  if [[ "$code" == "404" ]]; then
    skip "current_status endpoint not found (HTTP 404) — may not be enabled on this session"
    return
  fi

  # The key fix in v8.19.8 was changing 204→200 so PWA's r.json() doesn't throw
  if [[ "$code" == "204" ]]; then
    ko "GET /api/sessions/{id}/current_status returned HTTP 204 (regression) — should be 200 per v8.19.8 contract; PWA r.json() will throw"
    return
  fi

  if [[ "$code" != "200" ]]; then
    ko "GET /api/sessions/{id}/current_status returned HTTP $code (expected 200): $(echo "$body" | head -c 200)"
    return
  fi

  # Verify body is non-empty JSON
  if ! echo "$body" | python3 -c "import json,sys;json.load(sys.stdin)" 2>/dev/null; then
    ko "current_status returned 200 but non-JSON body: $(echo "$body" | head -c 200)"
    return
  fi
  ...
  ok "GET /api/sessions/{id}/current_status returns HTTP 200 with JSON body (no_change=$no_change) — v8.19.8 contract satisfied"
}
