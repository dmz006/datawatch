#!/usr/bin/env bash
# TS-489 — datawatch llm models list <name> exits 0
# tags: surface:cli feature:llm-registry
source "$(dirname "${BASH_SOURCE[0]}")/lib.sh"
CURRENT_STORY="TS-489"
...
_story_ts_489() {
  ...
  local out rc
  out=$(cli_test llm models list "$llm_name" 2>&1); rc=$?
  save_evidence TS-489 "out.txt" "$out"
  if [[ $rc -eq 0 ]]; then
    ok "datawatch llm models list $llm_name exits 0"
