...
_story_ts_320() {
  # rtk check lives under 'setup rtk check', not as a top-level command.
  local out; out=$(cli_test setup rtk check 2>&1); local rc=$?
  save_evidence TS-320 "out.txt" "$out"
  if [[ $rc -eq 0 ]]; then
    ok "datawatch setup rtk check exits 0"
