...
CURRENT_STORY="TS-333"
story_preflight "surface:cli feature:cli feature:parity" || return 0

_story_ts_333() {
  local out; out=$(cli_test identity show 2>&1); local rc=$?
  save_evidence TS-333 "out.txt" "$out"
  if [[ $rc -eq 0 ]]; then
    ok "identity show exits 0"
  elif echo "$out" | grep -qiE "disabled|not.*enabled|not.*configured|not.*found|no.*available|unknown command"; then
    skip "identity not configured: $(echo "$out" | head -c 80)"
  else
