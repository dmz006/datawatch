#!/usr/bin/env bash
# TS-607 — 5 locale bundles contain server_picker_federation_peer key
# tags: surface:locale feature:multiserver
source "$(dirname "${BASH_SOURCE[0]}")/lib.sh"
CURRENT_STORY="TS-607"
story_preflight "surface:locale feature:multiserver" || return 0

_story_ts_607() {
  local locale_dir="$REPO_ROOT/internal/server/web/locales"
  local check_key="server_picker_federation_peer"
  local count=0
  ...
  save_evidence TS-607 "count.txt" "$count"
  if [[ "$count" -ge 5 ]]; then
