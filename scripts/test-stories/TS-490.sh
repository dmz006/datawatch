#!/usr/bin/env bash
# TS-490 — datawatch llm refresh-models <name> exits 0
# tags: surface:cli feature:llm-registry
source "$(dirname "${BASH_SOURCE[0]}")/lib.sh"
CURRENT_STORY="TS-490"
...
_story_ts_490() {
  ...
  local out rc
  out=$(cli_test llm refresh-models "$llm_name" 2>&1); rc=$?
  save_evidence TS-490 "out.txt" "$out"
  if [[ $rc -eq 0 ]]; then
    ok "datawatch llm refresh-models $llm_name exits 0"
