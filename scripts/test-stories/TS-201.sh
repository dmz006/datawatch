...
_story_ts_201() {
    ...
    count=$(echo "$resp" | python3 -c "import json,sys; d=json.load(sys.stdin); items=d.get('llms',d) if isinstance(d,dict) else d; print(len(items) if isinstance(items,list) else 0)" 2>/dev/null || echo "0")
    if [[ "$count" -gt 0 ]]; then
