...
_story_ts_323() {
  local out; out=$(cli_test pipeline list 2>&1); local rc=$?
  save_evidence TS-323 "out.txt" "$out"
  if [[ $rc -eq 0 ]]; then
    ok "pipeline list exits 0"
  elif echo "$out" | grep -qiE "disabled|not.*enabled|not.*configured|not.*found|no.*available|unknown command"; then
    skip "pipeline list not available: $(echo "$out" | head -c 80)"
  else
