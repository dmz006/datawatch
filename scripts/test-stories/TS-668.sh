...
_story_ts_668() {
  ...
  cfg_code=$(api_code PUT /api/config \
    '{"vision.enabled":true,"vision.backend":"ollama","vision.endpoint":"http://localhost:11434","vision.model":"moondream:latest"}' \
...
" 2>/dev/null || true)
  ...
  # Give the daemon time to run vision describe (moondream can take 30-60s under load)
  sleep 60
