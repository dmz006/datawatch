...
_story_ts_317() {
  ...
  # Add
  out=$(cli_test llm add --name "$llm_name" --kind openai --api-key "sk-test" --model "gpt-4o-mini" 2>&1); local rc=$?
  ...
  # Show
  out=$(cli_test llm show "$llm_name" 2>&1) || out=$(cli_test llm get "$llm_name" 2>&1)
