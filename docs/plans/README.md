...
Latest release: **v5.27.5** (2026-04-29, patch — claude permission_mode + model + effort surfaces, BL206 frozen, plan-mode for PRDs).

| Bucket | Count | Notes |
|---|---|---|
| Open bugs | 2 | **BL211** scrollback state-detection · **BL215** rate-limit miss + no-reset-time fallback (operator repro 2026-04-30 — limit not detected AND no reset time in claude's message). Both ship as v5.27.6 hotfix bundle. |
| Open features | 7 | datawatch#26 (Running pulse + dots, BL208) · #27 (`📜` icon, BL208) · #28 (config-driven quick cmds, BL209) · #29 (channel.js MCP memory, BL212) · #30 (PRD card alignment, BL208) · #31 (Signal device-linking API, BL213) · #32 (PWA i18n DE/ES/FR/JA, BL214). |
| Active backlog | 9 | **BL211** scrollback state-detection (v5.27.6 hotfix) · **BL215** rate-limit miss + no-reset-time fallback (v5.27.6 hotfix) · **BL208** UI parity bundle (#26+#27+#30) · **BL209** config-driven quick commands (#28) · **BL210** daemon-MCP parity audit · **BL212** channel.js bridge MCP memory tools (#29) · **BL213** Signal device-linking API (#31) · **BL214** PWA i18n DE/ES/FR/JA (#32) · BL190 cosmetic · BL173-followup. |
...
| **BL213** | **Signal device-linking API** (datawatch#31). Mobile companion BL21 needs `POST /api/link/start` + `GET /api/link/qr` (SSE QR stream) + `GET /api/link/status` + `DELETE /api/link/{deviceId}`. ~half day. | Open — v5.28.x. |
| **BL214** | **PWA i18n** (datawatch#32) — extract all hardcoded English strings to `locales/en.json`, ship DE/ES/FR/JA matching Android v0.52.0 BL15 translations. Adopt a lightweight i18n store; wire `Accept-Language` detection + Settings override. ~1-2 days. | Open — v5.28.x or v6.x. |
