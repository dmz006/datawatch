...
Latest release: **v8.20.10** (2026-09-07). fix(autonomous): generalize one-shot PRD task delivery to all TUI backends (goose, aider, …) — previously only opencode received the task prompt via send_input. Deny-list excludes claude-code (channel bridge), opencode-acp (ACP API), subprocess (shell wrapping). Also v8.20.9: DATAWATCH_COMPLETE instruction in task payload; session filter via task.session_id; sprint synthesis from PRD.
