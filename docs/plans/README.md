...
Latest release: **v8.33.23** (2026-09-16). fix(autonomous): `reset_task` now works on PRDFailed PRDs (B97) — previously refused with "not running"; now accepts PRDFailed and auto-restores PRDRunning so partially-complete PRDs can be recovered without discarding all completed task work.
