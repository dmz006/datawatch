...
> **BL353** — Recurring named schedule. `schedule_add session_name="<name>"` that targets whichever session currently holds that name, surviving session death/restart. Add `cron=` field to `schedule_add` for cron-expression recurring schedules (currently one-shot only). Surviving schedules re-fire when a new session claims the same name. 7-surface parity: REST + MCP + CLI + comm + PWA + YAML.

> **BL354** — Name-addressed session operations. `send_input`, `session_output`, `kill_session`, and related MCP tools already accept short IDs and human-readable names via `resolveSession`. Document and test name-addressing explicitly; resolve race conditions where two sessions briefly share a name during handoff (return the one in the older non-terminal state, not an error). Add `session_name` param alias on all session-targeted MCP tools and REST endpoints.

> **BL355** — Session crash / zombie detection. Distinguish "tmux alive but AI process exited" from "session waiting for input". Add `claude_alive bool` field to session JSON (returned by `list_sessions`, `GET /api/sessions`), set by a periodic probe against the tmux pane PID. Surface in `list_sessions` output, alert on transition from alive=true to alive=false. 7-surface parity: REST + MCP + CLI + comm + PWA + mobile push.

> **BL356** — Session crash/exit hook. `on_session_exit` config: per-session-name hooks that trigger when `claude_alive` flips false or session enters failed/killed state unexpectedly. Actions: `restart` (relaunch with last task) or `notify` (send input to another named session). Config: `session.exit_hooks[].name`, `action`, `notify_session`, `cooldown`. 7-surface parity: YAML + REST (CRUD) + MCP + CLI + comm + PWA.

> **BL357** — Durable role-based work queue. `queue_push role=<name> payload={…}` / `queue_claim role=<name> lease=<sec>` / `queue_complete id=<id>` / `queue_fail id=<id>` / `queue_list role=<name>`. Atomic claim with lease expiry (unclaimed items become claimable again after lease). WAL-backed, host-durable. Any session can push; whichever session holds the role can claim. 7-surface parity: REST + MCP + CLI + comm + PWA + mobile.

> **BL358** — Discussion push / subscribe. `discussion_subscribe discussion_id=<id> session_name=<name>` delivers new WAL entries to the named session as `send_input` on arrival. Supports long-poll variant: `memory_discussion_wal discussion_id=<id> after_seq=N block=true timeout=60`. 7-surface parity: MCP + REST + CLI + comm.

> **BL359** — `restart_session` from any state. Allow `restart_session` when session is in `running`, `waiting_input`, or any other non-terminal state. Semantics: kill underlying process, relaunch with same or supplied task, keep same session ID and name. 7-surface parity: REST + MCP + CLI + comm + PWA.

> **BL360** — Structured agent result store. `result_put name=<str> payload={…}` / `result_get name=<str>` / `result_list prefix=<str>`. Small KV store for sub-agent result JSONs. WAL-backed, host-durable, optional TTL. 7-surface parity: REST + MCP + CLI + comm.

> **BL361** — `list_sessions` structured filters. Add query params / MCP params: `name=<glob>`, `state=<state>`, returning full structured JSON with `claude_alive` field (see BL355). Current text output from `list_sessions` is regex-parsed by callers; structured JSON closes this gap. 7-surface parity: REST + MCP + CLI.
