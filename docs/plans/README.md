...
| BL190 follow-up | **Full coverage shipped v5.14.0** — every one of the 13 howtos now has at least one PNG inlined; recipe map expanded from 6 to 11 (added `settings-monitor`, `settings-about`, `alerts-tab`, `autonomous-new-prd-modal`, `session-detail`). 11 PNGs in `docs/howto/screenshots/`, mirrored into the embedded PWA docs viewer via `make sync-docs`. Per-howto density (1-3 PNGs each) is intentionally lower than the 15-20-per-howto target in the original plan doc — the load-bearing pipeline is in place; further density is a mechanical follow-up. | ✅ Closed v5.14.0 (full coverage); per-howto density expansion stays an iterative cosmetic follow-up. |
