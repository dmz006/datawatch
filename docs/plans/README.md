...
| ID | Item | Notes |
|----|------|-------|
| BL173-followup | **Production validation** of Shape C — `make ebpf-gen` on a host with clang + linux-headers, `make cluster-image VERSION=v4.7.0`, deploy via Helm with `observer.shapeC.enabled=true`. Manifest already validated server-side against the testing cluster. | Operator-side; checklist in [`2026-04-25-bl173-followup-checks.md`](2026-04-25-bl173-followup-checks.md). |
| BL174 verification | **Measure agent-claude + agent-opencode size before/after v4.6.0** — operator must rebuild the images locally (no docker on dev laptop) and capture the numbers in a CHANGELOG follow-up. | Operator-side. |
...
| S13 follow — orchestrator integration | **Per-node `observer_summary` on BL117 graphs**. Design ✅ filed 2026-04-25 in [`2026-04-25-s13-followup-orchestrator-observer.md`](2026-04-25-s13-followup-orchestrator-observer.md). One field on `orchestrator.Node` (`AgentID`), executor wires it on spawn, server-side join attaches `observer_summary` to graph response. Implementation pending — small (~1 day). | v4.7.x patch or v4.8.0. |
| S14a — cross-cluster federation | **Multi-primary federation tree** — a primary datawatch can also be a peer of a *root* primary, giving operators with multiple clusters one pane of glass. Design ✅ filed 2026-04-25 in [`2026-04-25-s14-cross-cluster-federation.md`](2026-04-25-s14-cross-cluster-federation.md). 5-task plan, loop-prevention via push-chain, reuses existing peer registry. | Target v4.8.0. |
...
| B44 (reverse parity) | **PWA sessions list — search icon toggle** matching the datawatch-app UX. The current "▴ filters" text pill replaced with a magnifying-glass icon; default OFF (filters hidden, session list takes the full window); click to reveal the filter input + backend badges + history toggle. | Operator-visible UX parity with mobile; small PWA-only change. |
