...
| BL184 | **opencode-acp chat-mode recognition lag** — primary recognition-lag fix shipped v4.8.20: extracted `markChannelReadyIfDetected(sessionId, lines)` helper in `app.js` that runs unconditionally on every `output` and `chat_message` WS event. Previously the readiness scan only ran when the connection-banner element was already in the DOM, missing the case where the operator opens the session right after the ACP became ready. Cached `state.channelReady[sessionId]` is now correct on next render even if the banner wasn't on screen at detection time. **Remaining (secondary):** thinking-message UX — opencode-acp emits thinking text that arrives right before the response and isn't matched by the existing collapsible regex (`^Thinking\.\.\.\s*\(reason\)$`). Needs the daemon to wrap thinking blocks in `<thinking>…</thinking>` (already supported in `formatChatContent` line 758) OR a relaxed regex. | Partial — primary done v4.8.20; thinking-UX deferred. |
