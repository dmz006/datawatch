...
Latest release: **v8.20.9** (2026-09-07). feat(autonomous): prd_id/task_id fields on sessions so PWA can filter by PRD and show active task; verifying/running_tests tasks now count toward progress and show ⟳/🧪/⛔ glyphs; "No active session record" bug fixed. Also v8.20.7: opencode TUI one-shot task delivery via state-change handler. v8.20.6: Start Planning button disables in-place; Cancel button warning.
