...
- PWA and other viewable things should not use bug, backlog, feature, or any internal names or numbers.  For example "no peers registered · deploy datawatch-stats --datawatch <url> --name <peer> on a Shape B host, or spawn an F10 agent (auto-peers in v4.7.0+)" mentions shape B and F10 and there are BL and F and B items throughout.  If something should be referenced use a 1-3 word description that has a link to the doc.  F10, Shape B, BLXX and similar should already have a rule in AGENT.md and in memory.  Clean up.
...
| BL179 — PWA sessions search-icon location | Operator feedback 2026-04-25: the B44 search-icon toggle landed inside the sessions-card header. **Should live in the top header bar, immediately to the left of the green daemon-status light**, so it's always reachable from any tab — not just the sessions view. Pure PWA move (`internal/server/web/index.html` + `app.js`). | Backlog. |
