...
| Recently closed | BL327–BL334 ✅ v8.2.0–v8.6.0; BL336 ✅ v8.9.17–v8.9.19; BL337 ✅ v8.9.20; BL338 ✅ v8.9.21; BL339 ✅ v8.9.22; BL340 ✅ v8.9.23; BL341 ✅ v8.9.24; BL342+BL343 ✅ v8.9.25; BL347 ✅ v8.10.0 | badge/chip, async decompose, push, channel routing, file service, discussion scopes, operational encryption; anti-clobber typing hold + queue; line-printing renderer lock; update self-update archive priority + channel co-update; container builder CVE fix; imap-mcp email command channel; MCP session name resolution + permission_mode; compute migrate CLI + federation peer health alerts; session lineage + cascade kill + reply_to_parent |
...
> **BL344** — Alert click navigates to session detail (GH#120). Tapping an alert entry on the Alerts tab that has an associated session should open that session's detail view directly. Currently the alert row opens an alert-detail overlay but offers no direct navigation to the linked session. Implementation: add a "Go to session →" affordance (icon button or chip) to the alert detail overlay that calls `openSession(alert.sessionId)` when `sessionId` is present. PWA-only; mobile parity tracked at datawatch-app. (Filed 2026-06-03.)

> **BL346** — FCM `session_state_changed` lifecycle events (GH#117). The FCM notification payload currently supports `type: session_complete` and `type: alert_fired`. Android client (and iOS once APNs lands via BL335) needs `type: session_state_changed` with fields `{session_id, old_state, new_state, task, short_summary}` so the push notification can show a contextual "Session foo moved to WaitingInput" message. Server-side: fire on every `Manager.transitionState` call that produces a state-change event. Client-side: new notification template + optional in-app toast. (Filed 2026-06-04.)

> **BL348** — PWA session tree view. Add a "Group by parent" toggle in the Sessions list toolbar. When enabled, sessions with `parent_id` are indented under their parent card rather than appearing at the top-level list position; orphaned children (parent deleted or missing) remain at the top level with a `⚠ orphan` indicator. Toggle state persists in localStorage. Complements `session_children` MCP tool and `GET /api/sessions?tree=1` REST endpoint added in v8.10.0. 7-surface parity: PWA + mobile (CLI already shows `↳ child of [<id>]`; REST tree endpoint already exists).

> **BL349** — `get_my_session_id` MCP tool. A zero-argument MCP tool that returns the `session_id` of the session running the current claude-code agent. Resolves by inspecting the `caller_session_id` field set at session start (already stored in session metadata). Agents can call `session_children`, `reply_to_parent`, or `start_session` with `caller_session_id` populated without relying on `$CLAUDE_SESSION_ID` env var, which is not always forwarded through subshells or non-claude backends. REST: `GET /api/sessions/self` under auth-token context; CLI: `datawatch session self`.

> **BL350** — Orphan lineage cleanup. When a session with `parent_id` set refers to a parent that no longer exists, the session is an orphan. Add: (1) `GET /api/sessions?orphaned=1` filter param returning sessions whose `parent_id` points to a non-existent session; (2) `session_list(orphaned=true)` MCP param; (3) optional system alert on daemon start or periodic sweep when orphaned sessions are detected. Does not auto-delete; operators decide. Related to BL351 cascade depth policy.

> **BL351** — Kill-children cascade depth policy. Document and enforce the depth semantics of `kill_children=true`: current implementation is depth-1 only — `Kill()` calls `GetChildren(parent.FullID)` once and kills each direct child, it does not recurse into grandchildren. Add `kill_children_recursive=true` option (separate field, default false = current depth-1 behavior) for operators who want full recursive teardown. Update CLI/MCP params and definitions manual to document both modes explicitly.

> **BL352** — Federation lineage parity. `GET /api/sessions/{id}/children` and `GET /api/sessions?tree=1` only resolve children stored on the local daemon. When a parent on host A spawns a child on federated peer host B (via federation routing), the child's `parent_id` is a foreign session ID invisible to the local store. Add cross-host child resolution via `GET /api/sessions/aggregated?parent_id=<id>` querying all registered servers and merging results; `session_children` MCP tool would use the same path. Depends on BL316 federation infrastructure.
