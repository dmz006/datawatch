...
Latest release: **v5.28.1** (2026-04-30, BL214 wave-2 i18n string-coverage extension + BL173-followup production-cluster verification doc).
...
| Active backlog | 1 | BL190 cosmetic (iterative). BL173-followup runbook landed in v5.28.1 (`docs/howto/federated-observer.md`); awaits operator-run check on production cluster. BL214 wave-2 string-coverage extension shipped v5.28.1; iterative string wire-up continues in v5.28.x. |
...
| BL173-followup | **Live cluster→parent push** is operator-side: dev-workstation parent isn't reachable from the testing-cluster pod overlay. Production deploys don't have this gap. | **Code path verified v5.28.1** + production-cluster runbook landed in `docs/howto/federated-observer.md` ("Production-cluster reachability check"). Awaits operator-run check inside a prod pod (curl + verify + cleanup steps documented). Failure-mode triage included (connection-error = network gap, 401/403 = auth/token plumbing). |
