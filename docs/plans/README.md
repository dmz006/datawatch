...
- 📋 **BL278-original (filed 2026-05-07)** — Add a theme toggle near the top of Settings → General so the operator can switch between light and dark themes. Both themes must be **ADA-compliant** (WCAG AA contrast targets at minimum: 4.5:1 for normal text, 3:1 for large text and UI elements). Constraint: light mode must preserve **datawatch intent and history** — same brand accent (#60a5fa-ish blue), same purple accent2 used distinctly, same colour-coded states (running green, waiting amber, failed red, rate-limited amber, complete grey), same affordance for stale-comms amber dot, same lifecycle strip / verdicts colour semantics. The current theme is dark-only; switching introduces a `data-theme="light"` attribute on `<html>` (or `prefers-color-scheme` aware) and a parallel set of CSS custom-property values that map to the light palette. Persist the choice in `localStorage` so it survives reloads. 7-surface parity check: PWA only — no API change. Mobile-Parity Rule: file a `dmz006/datawatch-app` issue so the Compose Multiplatform companion gets the same toggle. Acceptance: every existing surface (sessions list, automata list/detail/wizard, settings, observer, council, secrets, identity wizard, profiles, rate-limit indicators) renders with WCAG AA contrast in both themes; the operator can toggle from any view with a single tap.
...
- 📋 **BL279-original (filed 2026-05-07)** — Embedded docs viewer: The `/diagrams.html` markdown viewer renders with **double-spaced paragraphs and headers** — too airy. Tighten paragraph + header spacing to a normal reading rhythm. Also: many of the docs lack inline cross-links to related material (e.g. a section in `datawatch-definitions.md` that talks about Council Mode should link directly to `howto/council-mode.md` and the relevant architecture / plan / API doc). Walk the docs and ensure every topic has the "see also" / "how to" / "architecture" / "API" links the operator needs to navigate without bouncing back to the index. **Implementation sketch:** (a) viewer CSS — drop `<p>` and `<h*>` margins from the current double-spaced default to ~0.6em / 0.4em; (b) doc walkthrough — add inline cross-references to every section that maps to a how-to or architecture doc (cross-reference matrix already in `datawatch-definitions.md` → "Core feature reference matrix" can drive the link insertion). Acceptance: viewer reads with normal density (no perceived double-space); every section in `datawatch-definitions.md` that references a feature has at least one inline link to the matching `howto/*.md` (and to `architecture*.md` when the section discusses internals). 7-surface parity: PWA viewer only — no API change. Mobile-Parity Rule: file a mobile issue if the companion has its own docs renderer.
...
- 📋 **BL277-original (filed 2026-05-06)** — Rip out the `needs-input-banner` (`app.js:878`, `style.css:3897-3970`, `dismissNeedsInputBanner` `app.js:807`, all `needs-input-*` CSS, all banner-related state and event handlers). The xterm input-bar's `.needs-input` yellow border (`style.css:2638`) is the visual cue and stays. **Keep** the `last_response` feature in full — the per-session WS-cached `state.lastResponse`, the &#128196; "View last response" icon on session cards (`app.js:2276`), `showResponseViewer`, the `last_response` field on `Session`. Operator: "useful for auto, watch and from sessions screen". Acceptance: zero `needs-input-banner` references in any surface; `last_response` API contract + UI chip + viewer untouched. 7-surface parity check: PWA only — no API change. Mobile-Parity: file a `dmz006/datawatch-app` issue if the mobile companion has its own banner.

- 🐞 **BL287 — PWA mic input regressed: no "processing" feedback + transcription doesn't land** (operator-filed 2026-05-07). Previous behavior: tap 🎤 in any input area → speak → tap stop → "processing…" indicator appears while the transcription backend (Whisper / configured voice backend) returns text → text lands in the input field, ready to send. Current behavior: no "processing" indicator appears AND the transcribed text never reaches the input field. Likely culprits: (a) the voice-input handler stopped wiring into the input element after a recent app.js refactor; (b) the backend response shape changed and the JS unmarshal silently drops the result; (c) the "processing" UI element was removed by a CSS sweep without restoring the JS hook. Surfaces affected: every input area that exposes the mic button (session input bar, automaton wizard intent textarea, every chat-mode input). **Repro:** open any session detail or the new-automaton wizard → tap the 🎤 button → speak → tap stop. Acceptance: (1) "Processing…" indicator visible from stop-tap until backend response, (2) transcribed text lands in the focused input field, (3) error state surfaces a clear toast if the backend fails. 7-surface check: PWA only — no API change unless the backend protocol drifted.

- 🐞 **BL288 — Settings → About card missing proper left + bottom padding** (operator-filed 2026-05-07). The About card on Settings → About reads flush against the card's left edge and runs to the bottom edge with no gutter. Should match the inset other Settings cards use (the v6.12.1 BL272 sweep set `.settings-section > .settings-row` + `div[id^="settings-sec-"]` to `padding: 8px 14px`). Likely cause: an inline-style override OR a non-standard wrapper around the About card body that bypasses the BL272 selector. Repro: Settings → About → observe Version / Daemon / Update / Language / Theme rows hugging the left edge and the last row touching the card's bottom border. Fix: ensure the About-card body wrapper inherits the standard `.settings-section`-child padding, or add explicit `padding: 8px 14px 12px` to whatever inner div renders the About rows.

- 📋 **BL289 — Document Ollama use across every datawatch feature that benefits from it; ensure no-GPU still works** (operator-filed 2026-05-07). Datawatch already supports `cfg.Ollama.Host` for one-spot configuration (operator's preferred deployment: dedicated GPU box on Tailscale). **Hard constraint — no feature may REQUIRE Ollama to function.** Every Ollama-using feature must degrade cleanly when Ollama is unreachable / not configured. Confirm + document: (1) every feature that CAN use Ollama uses it through `cfg.Ollama.Host` (memory embedder, BL274 vector index, council mode LLM, evals llm_rubric grader, autonomous decompose, etc.); (2) `docs/install-ollama-host.md` enumerates every feature → which model/endpoint it hits → what GPU advantage → **and what the degraded-mode behavior is when Ollama is absent**; (3) every Ollama-using code path has a tested fallback (BL274 vector layer falls back to BM25 — pattern to mirror everywhere). Sprint 2 of BL274 demonstrates the fallback pattern; subsequent feature audits replicate it.

- 🐞 **BL290 — `datawatch-stats --help` shows single-dash flag form ("-datawatch", "-inseure-tls") instead of double-dash ("--datawatch", "--insecure-tls"); also typo in "inseure"** (operator-filed 2026-05-07). The standard Go `flag` package emits single-dash flag names by default in `-h` output, but the docs (`docs/install-ollama-host.md`) show `--datawatch …`, `--name …`, `--insecure-tls`. Operators copy-pasting from the help text get the single-dash form which Go's flag package does accept, but it's inconsistent and confusing — and the docs use double-dash. **Two fixes in `cmd/datawatch-stats/`:** (1) switch from stdlib `flag` to `pflag` (or wrap the help printer) so `--name` is the canonical form in help and accepted on the command line; (2) fix the typo `-inseure-tls` → `-insecure-tls` (missing 'c'). Acceptance: `datawatch-stats --help` shows `--datawatch`, `--name`, `--token-file`, `--push-interval`, `--listen`, `--insecure-tls` (all double-dash); single-dash form continues to work for backward compat.

- 🐞 **BL291 — Observer settings not findable in PWA; need a dedicated card or sub-tab** (operator-filed 2026-05-07). Operator could not locate where to manage federated observer config + peer registry from the PWA. The Federated Peers card is on Settings → Monitor, but observer-mode toggle, push targets, and peer registry CRUD aren't all in one place. Confirm + add: (1) one Settings card that contains all observer config (mode A/B/C, peer registry CRUD, push interval, parent URL); (2) cross-link from Settings → Monitor → Federated Peers card to that settings card; (3) one-line splash on first PWA load if the daemon is shape A/B/C and operator hasn't visited the observer settings yet. 7-surface parity: REST + MCP + CLI + comm already cover the operator-set; the gap is PWA + mobile only. Mobile-Parity: file matching `dmz006/datawatch-app` issue once the PWA card lands.
