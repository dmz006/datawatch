...
Latest release: **v8.20.11** (2026-09-07). fix(autonomous): pane-watcher scrollback scan for DATAWATCH_COMPLETE on one-shot TUI sessions — opencode's progress animation pushed the completion marker into scrollback before the 3s watcher fired; new CapturePaneScrollback fallback catches it. Also v8.20.10: generalized TUI task delivery to all backends (goose/aider/etc).
