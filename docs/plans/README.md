...
| ✅BL97 | Agent diaries (mempalace per-agent wing) for F10 workers | shipped | New `memory.AppendDiary(store, agentID, projectDir, room, hall, content)` writes via SaveWithMeta with the canonical convention `wing="agent-<id>"`, `role="diary"`, default `hall="events"`. New `memory.ListDiary(store, agentID, limit)` returns the agent's entries chronologically, filtered by wing. Survives worker termination + queryable for retrospectives. 8 tests cover wing naming, validation, defaults, isolation, ordering, limit clamping. |
| ✅BL98 | Contradiction detection (mempalace fact_checker port) | shipped | New `memory.SetFunctionalPredicates([]string)` + `KnowledgeGraph.FindContradictions()` flag (subject, predicate) sets where two or more active triples exist with different objects on a registered functional predicate. Operator-tunable predicate set (default empty = no detection — pre-BL98 behaviour preserved). `ResolveContradictionLatestWins(c)` invalidates older triples in a conflict set. 7 tests. |
| ✅BL99 | Closets/drawers (mempalace verbatim→summary chain) | shipped | New `memories.drawer_id` column links a closet (summary, embedded, search target) to its drawer (verbatim, no embedding). New `SaveClosetWithDrawer(ctx, store, embedder, projectDir, verbatim, summary, role, sessionID)` writes both rows + the link in one call. New `Store.Drawer(closetID)` returns the verbatim row (or nil for standalone closets). 6 tests cover happy path + standalone + missing-arg paths. |
...
| ✅BL115 | Pre-release K8s functional test suite (documented) | shipped | Live BL114+BL112 K8s smoke run against operator's `testing` 3-node cluster + read-only NFS probe (no operator data modified). Per-feature verdict matrix for every backlog item shipped this batch (24 of 24 PASS — all unit-test verified, BL112+BL114 also live-verified). Operator-pass items (image build/push, spawn_docker.sh, spawn_k8s.sh with RUN_BOOTSTRAP=1, helm install --dry-run, UI walkthrough) documented for release host with concrete commands. Captured in `docs/testing.md` "Release Checkpoint — F10 (v3.0.0 candidate)" section. **No regressions detected; recommend release after operator-pass items complete.** |
| ✅BL116 | Sessions list: badge for scheduled commands | shipped | New `ScheduleStore.CountForSession(id)` + REST `/api/sessions` decorates each entry with `scheduled_count`; comm `session list` appends `📅 N scheduled` after the optional `⚠ INPUT` flag. Web-UI render alongside other badges is the BL116-followup (data path is ready). |
