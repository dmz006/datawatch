...
| ID | Item | Notes |
|----|------|-------|
| BL173-followup | **Production validation** of Shape C — `make ebpf-gen` on a host with clang + linux-headers, `make cluster-image VERSION=v4.6.0`, deploy via Helm with `observer.shapeC.enabled=true`. Manifest already validated server-side against the testing cluster. | Operator-side; checklist in [`2026-04-25-bl173-followup-checks.md`](2026-04-25-bl173-followup-checks.md). |
...
| BL174 verification | **Measure agent-claude + agent-opencode size before/after v4.6.0** — operator must rebuild the images locally (no docker on dev laptop) and capture the numbers in a v4.6.x note. | Operator-side. |
