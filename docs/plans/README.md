...
| BL191 (PWA + Q4 + Q6) | **First cut shipped v5.2.0** — Q1 review/approve gate (status machine + Approve/Reject/RequestRevision/EditTaskSpec) + Q3 decisions log + Q2 templates (IsTemplate flag + InstantiateTemplate with `{{var}}` substitution) all live across REST + CLI + chat + MCP. Still open per Q5: full PWA CRUD UI for the PRD lifecycle + LLM dropdown with model + effort (task #283). Q4 recursive child-PRDs via the BL117 orchestrator + Q6 guardrails-at-all-levels remain. | Open (Q1+Q2+Q3 shipped v5.2.0; Q4+Q5+Q6 remain). |
...
| BL200 howto coverage expansion | **In flight (v5.3.0)** — operator directive 2026-04-26: refresh the existing 6 howtos against everything shipped since v4.9.3, add a setup+install howto, add a chat+LLM quickstart, audit every shipped feature and add a howto for any semi-complex one missing. Target ~12-15 howtos total. Plan + scope mirrored at [`2026-04-26-bl190-howto-screenshot-rebuild.md`](2026-04-26-bl190-howto-screenshot-rebuild.md). | In progress. |
| BL201 voice/whisper backend inheritance | **Open** — operator note 2026-04-26: voice transcription should reuse already-configured ollama / openwebui / openai endpoint+key from the LLM config block instead of asking the operator for a separate endpoint+key pair. PWA dropdown landed v5.2.0; the daemon-side resolution stays. Includes: (a) NewFromConfig looks up the selected backend's existing endpoint+key; (b) PWA hides endpoint+key fields when backend != whisper; (c) test button posts a 1KB silent WAV through the transcriber and refuses to mark `whisper.enabled=true` if the probe fails. | Queued (post-BL200). |
...
| BL203 flexible LLM selection (per-task / per-PRD) | **Backend + REST + CLI + chat + MCP shipped v5.4.0** — operator directive: "i want to be able to use the right llm for autonomous operations so it needs to be flexible". Most-specific wins: per-task → per-PRD → per-stage (decomp/verify) → global session.llm_backend. New `Task.{Backend,Effort,Model}` + `PRD.Model` fields + `POST .../{id}/{set_llm,set_task_llm}` endpoints + `prd-set-llm` + `prd-set-task-llm` CLI + `set-llm` / `set-task-llm` chat verbs + `autonomous_prd_set_{llm,task_llm}` MCP tools. **Still open**: PWA dropdowns on the New-PRD modal + per-task expansion. | Open (parity surfaces shipped v5.4.0; PWA wiring follows BL202). |
