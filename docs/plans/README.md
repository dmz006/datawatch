...
Latest release: **v8.33.20** (2026-09-16). fix(pwa/autonomous): progress bars show 0% despite completed tasks (B94-related); story status `"running"` injected by LLM decomposer now normalized to `StoryPending` in `SetStories` (B95). `_renderStatusGraphs` in app.js adds `'completed'` to `terminalTaskStates` (was `'done'`, never matched executor's `TaskCompleted`); also handles `'running'` story status for active indicator. B95: `SetStories` now forces `stories[i].Status = StoryPending` regardless of LLM output.
