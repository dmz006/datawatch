...
Latest release: **v8.33.19** (2026-09-16). fix(autonomous): executor skips re-spawning when TaskVerifying session is still live on daemon restart (B93). `executeOne` now checks `sessionAliveFn` at attempt==0; if alive, skips spawn and reconnects directly to the verify wait. Prevents duplicate sessions on every daemon restart when a task was mid-execution.
