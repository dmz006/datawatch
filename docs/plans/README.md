...
| ID | Item | Notes |
|----|------|-------|
| BL170 | **Feature-completeness audit** — every shipped feature has operator doc + OpenAPI (× 2 copies) + MCP tool + CLI + comm + flow diagram (where applicable). Seeded by [#16](https://github.com/dmz006/datawatch/issues/16). | **✅ shipped through v4.5.0** — 5 phases: gap inventory + 90.5% openapi coverage (115/127) + 5 new flow diagrams + arch refresh + MCP mapping refresh. v4.5.1 closed the BL172 peer-registry MCP/CLI/comm gap. Audit doc: [`2026-04-25-bl170-feature-completeness-audit.md`](2026-04-25-bl170-feature-completeness-audit.md). |
| BL174 | **Native Go MCP channel server + slim claude container**. Replace `channel.js` with a Go binary using `mark3labs/mcp-go`; bundle into agent-base. | **✅ shipped through v4.4.0** — Go bridge in v4.3.0; `datawatch setup channel --cleanup` migration; agent-base bundle in v4.4.0. Design: [`2026-04-25-bl174-go-mcp-channel-and-slim-container.md`](2026-04-25-bl174-go-mcp-channel-and-slim-container.md). Distroless / alpine spike still deferred. |
| BL171 / BL172 / BL173 | **datawatch-observer — unified stats / process-tree / sub-process monitoring**. Three-shape deployment (in-process / standalone daemon / cluster container), one wire contract. GitHub [#20](https://github.com/dmz006/datawatch/issues/20). | **All three shapes ✅ shipped**: S9 / Shape A in v4.1.0, S11 / Shape B in v4.4.0, S12 / Shape C in v4.5.0 (manifest validated against live testing cluster; k8s metrics-server scraper validated against real PKS payload). Designs: [BL171](2026-04-22-bl171-datawatch-observer.md), [BL172](2026-04-25-bl172-shape-b-standalone-daemon.md), [BL173](2026-04-25-bl173-shape-c-cluster-container.md). **Production verification checklist**: [`2026-04-25-bl173-followup-checks.md`](2026-04-25-bl173-followup-checks.md). |

### Open (deferred / awaiting operator action)

| ID | Item | Notes |
|----|------|-------|
| BL173-followup | **Production validation** of Shape C — `make ebpf-gen` on a host with clang + linux-headers, `make cluster-image VERSION=v4.5.0` with registry push, deploy via Helm with `observer.shapeC.enabled=true`, verify `host.ebpf.kprobes_loaded=true` + `cluster.nodes[]` populated. Manifest already validated server-side against the testing cluster. | Operator-side; checklist in [`2026-04-25-bl173-followup-checks.md`](2026-04-25-bl173-followup-checks.md). |
| BL173 task 6 | **PWA process-tree drill-down panel** — replace the toast in `showObserverPeerSnapshot` with a proper modal showing the full envelope tree. `/api/observer/envelope` already returns the data. | UI polish; operator-visible. |
| BL173 follow | **Fold k8s metrics scraper output into StatsResponse v2** — the `K8sMetricsScraper` lands in v4.5.1 alongside the parser tests, but `cluster.nodes[]` in the snapshot still comes from the lazy collector path; need to wire the live scraper output. | Code-only; small. |
| BL174 stretch | **Distroless / alpine spike for agent-base** — would shrink ~250 MB image. Deferred until image-size telemetry shows headroom worth chasing. | Defer. |
| S13 (next sprint) | **Agent / recursive drill-down trees** — merges observer with F10 ephemeral agents + BL117 orchestrator; out of scope until mobile v1.0 lands AND Shape C runs in production. | Future v4.6.0+. |
| Mobile parity (#4) | datawatch-app Compose Multiplatform follow-ups: see [datawatch-app#2](https://github.com/dmz006/datawatch-app/issues/2) (federated peers card), [#3](https://github.com/dmz006/datawatch-app/issues/3) (cluster nodes card), [#4](https://github.com/dmz006/datawatch-app/issues/4) (eBPF status badge), [#5](https://github.com/dmz006/datawatch-app/issues/5) (native plugins display). | External repo. |
| v4.5.1 binary upload | GitHub release-asset upload API was unresponsive on 2026-04-25 (graphql EOFs + multi-min hangs on single-file uploads). Release published code-only with source tarballs auto-attached. **Daemon ABI unchanged from v4.5.0**, so existing v4.5.0 binaries work for everything except the new MCP / CLI / chat surfaces. | Re-attempt when the upload endpoint recovers. |
