...
Latest release: **v5.28.2** (2026-04-30, BL173-followup CLOSED via testing-cluster end-to-end verification).
...
| Active backlog | 1 | BL190 cosmetic (iterative). BL173-followup **closed v5.28.2** — verified in the operator's testing cluster end-to-end (peer pod → ClusterIP Service → parent pod cross-node, register/push/aggregate/cleanup all `status:ok`). BL214 wave-2 string-coverage extension shipped v5.28.1; iterative string wire-up continues in v5.28.x. |
