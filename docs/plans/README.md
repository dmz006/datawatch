...
Latest release: **v8.33.21** (2026-09-16). fix(pwa): image attachment success toast (B94) — `showToast` added to `onSessionImageSelected` success handler so users see "Image attached — will be sent with your next message" after upload completes. Also closes B90 README entry (was stale; `reconcileStuckTasks` shipped in v8.33.2 is the fix, corrected release note from TaskFailed→TaskPending).
