...
| S13 follow — orchestrator integration | **Per-node `observer_summary` on BL117 graphs** — needs orchestrator Node to carry `agent_id` first. Spec'd in [`2026-04-25-s13-agent-recursive-drilldowns.md` §5](2026-04-25-s13-agent-recursive-drilldowns.md), but the orchestrator change is a separate task. | Tracked; small but needs orchestrator-side change. |
