...
Latest release: **v8.33.25** (2026-09-16). fix(pwa/autonomous): three Automata detail-view bugs — (B99) `ResetTask` now resets story status from `completed`/`failed` to `pending` when resetting a task within that story; progress card dot no longer shows ✓ for a story when any of its tasks are actively running; PRD tab switching (Overview/Stories/Decisions) now swaps tab content in-place without re-fetching the PRD, eliminating the loading flash on every tab click.
