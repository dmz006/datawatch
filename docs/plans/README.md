...
| ID | Item | Status |
|----|------|--------|
| BL180 Phase 2 (kprobes + cross-host) | **Procfs cut v5.1.0; cross-host shipped v5.12.0; eBPF kprobes still open.** `Callers []CallerAttribution` envelope shape + procfs userspace correlator + new `Envelope.ListenAddrs` + `Envelope.OutboundEdges` + new `CorrelateAcrossPeers` federation aggregator that joins peer-A's session outbound conns to peer-B's backend listeners with `<peer>:<envelope-id>` caller prefix. Reachable via `GET /api/observer/envelopes/all-peers` + `observer_envelopes_all_peers` MCP tool + `datawatch observer envelopes-all-peers` CLI. 7 new unit tests cover happy path, wildcard listener, same-peer-not-matched, single-peer no-op, sort-order, unmatched-edge, and local-peer-prefix-suppression. Still open: eBPF kprobe layer (Q1 — `__sys_connect` + `inet_csk_accept` + `conn_attribution` LRU map + userspace TTL pruner; backed out cleanly v5.6.0). Thor smoke-test still needs a host the operator provisions (Q6). | Cross-host shipped v5.12.0; eBPF remains. |
...
| BL190 follow-up | **Capture pipeline + first cut shipped v5.11.0** — `scripts/howto-shoot.mjs` (puppeteer-core driver against `/usr/bin/google-chrome` headless; recipes for sessions / autonomous / settings sub-tabs / diagrams) + `scripts/howto-seed-fixtures.sh` (idempotent JSONL seed for PRD / orchestrator / pipeline stores) + 6 representative screenshots inlined into 4 howtos (chat-and-llm-quickstart, autonomous-planning, voice-input, mcp-tools). Operator removed the chrome MCP plugin (suspected memory issue); the new pipeline runs entirely outside the chrome MCP via puppeteer-core in `/tmp/puppet`. Remaining work: extend the recipe map to ~15-20 shots per howto across the full 13-doc suite (~200 PNGs total). | First cut shipped v5.11.0; expand-and-fill remaining. |
