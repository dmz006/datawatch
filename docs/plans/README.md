...
Latest release: **v8.20.7** (2026-09-07). fix(autonomous): opencode one-shot task prompt never delivered (no channel bridge to call /api/channel/ready); state-change handler now delivers task via send_input when opencode one-shot session reaches waiting_input. Also v8.20.6: Start Planning button disables in-place; Cancel button shows planning-specific warning.
