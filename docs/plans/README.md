...
| BL191 (PWA + Q4 + Q6) | **Q1+Q2+Q3 shipped v5.2.0; Q4 shipped v5.9.0; Q6 remains.** Q1 review/approve gate (status machine + Approve/Reject/RequestRevision/EditTaskSpec) + Q3 decisions log + Q2 templates all live across REST + CLI + chat + MCP. Q4 recursive child-PRDs (option (a) shortcut from the design doc): `Task.SpawnPRD` flag + `PRD.ParentPRDID/ParentTaskID/Depth` genealogy + `Config.MaxRecursionDepth` (default 5) + `Config.AutoApproveChildren` (default true) + new `autonomous_prd_children` MCP tool + `datawatch autonomous prd-children` CLI + `autonomous children <id>` chat verb. Q6 guardrails-at-all-levels remains open. | Open (Q1+Q2+Q3+Q4 shipped; Q6 remains). |
