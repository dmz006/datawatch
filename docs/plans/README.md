...
| BL113 | Self-managing platform bootstrap (host + cluster install paths) | 2-3 days | Document + automate the secured initial install for both single-host (systemd, `~/.kube/config` + `gh auth import`) and cluster (Helm chart + ServiceAccount + Secret/SealedSecret + RBAC) paths. End state: operator runs one command, datawatch boots itself in the cluster, manages its own ephemeral sessions, and reads kubectl/git creds from cluster Secrets — never from the operator's home dir at runtime. Touch points: Helm values overrides, Secret templates, install script, `docs/registry-and-secrets.md` walkthrough, post-install smoke. Pairs with BL111 (Vault/CSI) and BL110 (MCP api/config) for full self-management. |
| BL114 | Shared NFS volume mount for cross-session work | 1 day | Add a "shared workspace" mount (NFS/CSI/PVC) addressable from every spawned agent so multiple sessions can pass artifacts, large datasets, or build caches without going through git. Surfaces: `ClusterProfile.SharedVolumes []{name, mount_path, csi/nfs/pvc spec}`; driver injection (Docker `-v` / K8s `volumeMounts`); per-agent mount validation (read-only vs rw); audit event when written. Default off for security. Doc in `docs/registry-and-secrets.md` once shipped. |
