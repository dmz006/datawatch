...
| Sprint | Status | Notes |
|--------|--------|-------|
| Sprint 1 | ✅ all stories shipped | F10 foundations + slim image |
...
**Backlog generated by F10 work** (queued behind Sprint 5):
- BL92 — Write-through session registry (filed during cdbb orphan investigation)
- BL93 — Startup session reconciler (same)
- BL94 — `datawatch session import <dir>` manual escape hatch (same)

Full per-story status is annotated inline below with `[x] shipped` / `[ ] pending` markers.
...
- **S1.1 — BL16: health endpoints** *(2h)* ✅ shipped
...
- **S1.2 — Dockerfile (full + slim variants)** *(1d)*  *(superseded by S1.7 image taxonomy 2026-04-18)* ✅ shipped
...
- **S1.3 — Workspace volume convention** *(2h)* ✅ shipped
...
- **S1.9 — Image taxonomy v2: per-agent + per-language (Pod composition)** *(2d)*  *(new 2026-04-18, supersedes S1.7)* ✅ shipped
...
- **S1.7 — Image taxonomy v1: agent-base + per-language** *(1.5d)*  *(superseded by S1.9 — kept for trace)* ✅ shipped
...
- **S1.8 — make container-upgrade** *(3h)*  *(new 2026-04-18)* ✅ shipped
...
- **S1.5 — Smoke test image with real session** *(2h)* ✅ shipped
...
- **S2.1 — Schema + storage** *(1d)* ✅ ✅ shipped
...
- **S2.4 — Settings UI: Project Profiles card** *(1d)* ✅ ✅ shipped
...
- **S2.5 — Settings UI: Cluster Profiles card** *(1d)* ✅ ✅ shipped
...
- **S3.1 — Spawn API + abstraction** *(1d)* ✅ shipped
...
- **S3.2 — Docker driver** *(1.5d)* ✅ shipped
...
- **S3.3 — Bootstrap endpoint** *(4h)* ✅ shipped
...
- **S3.4 — Worker self-registration** *(4h)* ✅ shipped
...
- **S3.5 — Reverse proxy for child** *(1d)* ✅ shipped
...
- **S3.6 — Session-on-worker binding** *(4h)* ✅ shipped
...
- **S4.4 — Helm chart for the parent** *(1d)* ✅ shipped
...
- **S4.5 — Cluster Profile: smoke that spawns a real Pod** *(4h)* ✅ shipped
...
- **S5.3 — Worker git clone on bootstrap** *(4h)* ✅ shipped
...
- **S6.5 — Cross-profile sharing** *(4h)* ✅ shipped (mutual-opt-in EffectiveNamespacesFor; server enforcement BL101)
...
- **S7.7 — Comm-channel inheritance** *(4h)* ✅ shipped (bootstrap delivers Comm.Channels list; worker proxy-send wiring is BL102)
...
- **S8.1 — Secrets pluggability** *(1.5d)* ✅ shipped (Provider interface + File/EnvVar concrete + K8sSecret/Vault/CSI stubs; CredsRef wiring + token-broker storage migration = BL111)
...
- **S8.3 — Multi-cluster** *(1d)* ✅ shipped (DefaultClusterProfile fallback; multi-context kubectl caching is shell-out semantics)
...
- **S8.5 — CF placeholder** *(2h)* ✅ shipped
...
- **S8.6 — Idle-timeout enforcement** *(3h)* ✅ shipped (NoteActivity + ReapIdle; sweeper goroutine wiring = BL108)
...
- **S8.7 — Crash policy** *(3h)* ✅ shipped (config + validation; runtime enforcement = BL106)
