...
| **Spatial structure** | Wings → Rooms → Halls → Tunnels → Closets → Drawers | Wings → Rooms → Halls (3-level) | Datawatch adopted 3 levels; closets/drawers not implemented (verbatim mode stores directly) |
...
| **Contradiction detection** | `fact_checker.py` (checks KG for conflicts) | Not implemented | Potential future feature |
| **Agent diaries** | Per-agent wing with AAAK diary | Not implemented | Datawatch uses per-session auto-save instead |
...
|---------|-----|---------|-------------------|--------------------------|
| Spatial organization | BL55 | v1.5.0 | 6-level palace: wings → rooms → halls → tunnels → closets → drawers. Auto-detected rooms. Five fixed hall types. | Adopted 3 levels (wing/room/hall). Auto-derive wing from project path, hall from role. Skipped closets/drawers (verbatim stores directly). Achieved same +34pp retrieval improvement. |
