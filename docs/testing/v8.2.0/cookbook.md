...
|---|---|---|---|
| TS-637 | PWA | Secrets tags field: type tag + Enter/comma → badge appears; × removes it | 📋 planned |
| TS-638 | PWA | Federation peer capabilities: dropdown shows known caps; select creates badge | 📋 planned |
| TS-639 | PWA | LLM fallback chain: drag badges to reorder; order reflected in API payload | 📋 planned |
| TS-640 | PWA | Compute node tags: badge input saves as comma-separated to REST | 📋 planned |
| TS-641 | PWA | Profile memory_shared_with: badge input saves and re-renders from saved value | 📋 planned |
| TS-642 | PWA | Profile skills: badge input saves and re-renders from saved value | 📋 planned |
| TS-643 | PWA | Badge input: empty submission shows placeholder, no empty badge added | 📋 planned |
| TS-644 | PWA | Badge input: known-set field filters dropdown on typing; no-match shows no dropdown | 📋 planned |
...
| TS# | Surface | Description | Status |
|---|---|---|---|
| TS-645 | REST | POST decompose returns 202 with task_id + stream_url | 📋 planned |
| TS-646 | REST | SSE stream emits story events then complete event | 📋 planned |
| TS-647 | REST | GET /decompose/status returns running → complete progression | 📋 planned |
| TS-648 | REST | Last-Event-ID replay: reconnect mid-stream receives only missed events | 📋 planned |
| TS-649 | REST | Idempotent: second POST for in-flight job returns 202 with same task_id | 📋 planned |
| TS-650 | CLI | datawatch autonomous prd decompose <id> exits 0 | 📋 planned |
| TS-651 | MCP | autonomous_prd_decompose tool returns task_id + stream_url | 📋 planned |
| TS-652 | PWA | Decompose button shows inline progress panel; stories appear incrementally | 📋 planned |
| TS-653 | PWA | Progress panel shows reconnecting state on forced disconnect | 📋 planned |
| TS-654 | Federation | CapAutonomousWrite required: peer lacking cap → 403 | 📋 planned |
| TS-655 | Federation | CapAutonomousRead required for status/stream: read-only peer → 503/200 not 403 | 📋 planned |
| TS-656 | REST | Status after completion: status=complete, stories array populated | 📋 planned |
| TS-657 | REST | Status for unknown PRD with no job: 404 | 📋 planned |
...
| TS# | Surface | Description | Status |
|---|---|---|---|
| TS-658 | REST | POST /api/identity returns current identity (same as PATCH) | 📋 planned |
| TS-659 | REST | POST /api/identity with partial body updates only those fields | 📋 planned |
| TS-660 | REST | GET /api/identity returns identity object with role/focus/notes | 📋 planned |
| TS-661 | REST | PUT /api/identity replaces whole identity | 📋 planned |
| TS-662 | Federation | CapConfigWrite required for POST/PUT/PATCH; ro peer → 403 | 📋 planned |
| TS-663 | Federation | CapConfigRead required for GET; monitor peer → 403 | 📋 planned |
| TS-664 | PWA | Settings → General → Identity card: save via POST updates daemon state | 📋 planned |
...
| TS# | Surface | Description | Status |
|---|---|---|---|
| TS-665 | REST | GET /.well-known/unifiedpush returns {version:1,unifiedpush:{gateway:...}} | 📋 planned |
| TS-666 | REST | POST /api/push/register with endpoint returns {ok:true,id:reg-...} | 📋 planned |
| TS-667 | REST | GET /api/push/register returns registrations list | 📋 planned |
| TS-668 | REST | DELETE /api/push/unregister by id removes registration | 📋 planned |
| TS-669 | REST | DELETE /api/push/unregister by endpoint removes registration | 📋 planned |
| TS-670 | REST | POST /api/push/notify sends to all endpoints; returns 202 | 📋 planned |
| TS-671 | REST | POST /api/push/notify with registration_id targets only that endpoint | 📋 planned |
| TS-672 | CLI | datawatch push list, push test, push unregister all exit 0 | 📋 planned |
| TS-673 | Federation | CapCommWrite required for register/unregister/notify; ro → 403 | 📋 planned |
...
| TS# | Surface | Description | Status |
|---|---|---|---|
| TS-674 | REST | GET /api/alert-rules returns valid schema (App#133 parity) | 📋 planned |
| TS-675 | REST | POST /api/alert-rules creates rule; Android can parse shape | 📋 planned |
| TS-676 | REST | GET /api/skills/registries first entry is community registry (App#134) | 📋 planned |
| TS-677 | REST | GET /api/plugins/browse?registry=community returns plugin list | 📋 planned |
| TS-678 | REST | POST /api/plugins/install returns task_id (App#134 install flow) | 📋 planned |
| TS-679 | REST | GET /api/identity returns role/focus/context_notes fields (App#133 identity card) | 📋 planned |
| TS-680 | REST | POST /api/push/register accepts shape from Android UP library | 📋 planned |
| TS-681 | REST | POST /api/voice/transcribe accepts audio/webm (App#135 mic overlay) | 📋 planned |
| TS-682 | REST | Mic overlay: POST /api/sessions/{id}/input accepts text from transcription | 📋 planned |
