...
sequenceDiagram
    ...
    Note over Manager,Hub: Hook event received (BL303 S3)
    Manager->>Hub: BroadcastHookUpdate("myhost-a3f2", board)
    Hub->>Browser: {"type":"hook_update","data":{"session_id":"myhost-a3f2","board":{...}}}
    Note over Browser: Status tab re-renders in real-time\nno 5s poll needed
```
