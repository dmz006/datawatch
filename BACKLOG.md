# bugs
- in web interface, if a backend is selected that is not installed/configured, should open a way to configure or edit it with proper instructions on how to setup (maybe link to github documentation if needed) currently shows a warning but no wizard or configuration or edit config optoin
- requets notification permission" on android chrome  "permission denied" 
