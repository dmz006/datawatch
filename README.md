...
**Current release: v5.28.0 (2026-04-30).** PWA i18n foundation (BL214, datawatch#32) — DE/ES/FR/JA translations sourced 1:1 from the datawatch-app Compose Multiplatform Android client (`composeApp/src/androidMain/res/values{,-de,-es,-fr,-ja}/strings.xml`) so PWA stays in lockstep with the mobile companion's vetted translations. Ships a zero-dep i18n harness (`window._i18n` + `t(key, vars)` helper supporting Android-style `%1$s/%1$d` placeholders), `navigator.language` auto-detection with `Accept-Language` fallback chain, manual override via Settings → General → Language picker (Auto / English / Deutsch / Español / Français / 日本語) persisted in localStorage. Initial coverage: bottom nav (Sessions/Autonomous/Alerts/Settings), Settings tabs (Monitor/General/Comms/LLM/About), `data-i18n` DOM-sweep helper for declarative wiring. Locale bundles embedded in the binary at `internal/server/web/locales/{en,de,es,fr,ja}.json` (~240 keys each); served as static `/locales/<lang>.json`. Coverage of the remaining ~9700 lines of `app.js` is iterative — v5.28.x patches will extend `t()` calls into modals/forms/dialogs as Android string set evolves. Pre-6.0 testing window. Carries forward: PRD card style harmonised with Sessions cards, 11 daemon-MCP gap-closure tools (memory_wal/test_embedder/wakeup, claude_models/efforts/permission_modes, rtk_version/check/update/discover, daemon_logs), PWA UI parity bundle (Running pulse + 3-dot generating indicator + 📜 scroll icon), config-driven `/api/quick_commands`, channel.js bridge memory tools, subsystem-aware `POST /api/reload?subsystem=<name>`, `session.claude_auto_accept_disclaimer`. The single canonical install guide is [`docs/howto/setup-and-install.md`](docs/howto/setup-and-install.md). Per-version detail in [CHANGELOG.md](CHANGELOG.md); cumulative release notes since v4.0.0 in [docs/plans/RELEASE-NOTES-v5.0.0.md](docs/plans/RELEASE-NOTES-v5.0.0.md).
