...
**Current release: v5.27.8 (2026-04-30).** PRD card style harmonised with Sessions cards (status-driven left border + same bg/radius/padding); 11 new MCP tools close the daemon-MCP coverage gaps the operator audit flagged (memory_wal/test_embedder/wakeup, claude_models/efforts/permission_modes, rtk_version/check/update/discover, daemon_logs). PWA UI parity bundle (Running pulse + 3-dot generating indicator + 📜 scroll icon to match Android), config-driven `/api/quick_commands` endpoint so clients drop hardcoded button lists, and channel.js bridge gains `memory_remember/recall/list/forget/stats` MCP tools so claude-code sessions can use the parent's memory subsystem without curl workarounds. Pre-6.0 testing window. Hotfix on top of v5.27.2 that wires the subsystem-reload chat-channel command on the test/message router (the v5.27.2 binary returned "Reload not wired by this build" via chat surfaces). Refactor: `claudeDisclaimerResponse` extracted as a pure helper for unit-testability — same behaviour. v5.27.2's headline features (`POST /api/reload?subsystem=<name>` and `session.claude_auto_accept_disclaimer`) carry forward. The single canonical install guide is [`docs/howto/setup-and-install.md`](docs/howto/setup-and-install.md). Per-version detail in [CHANGELOG.md](CHANGELOG.md); cumulative release notes since v4.0.0 in [docs/plans/RELEASE-NOTES-v5.0.0.md](docs/plans/RELEASE-NOTES-v5.0.0.md).
