...
**Current release: v6.7.0 (2026-05-04).** Minor: **BL255 Skill Registries** — built-in PAI default registry (`danielmiessler/Personal_AI_Infrastructure`), full CRUD across REST/MCP/CLI/comm/PWA per the Configuration Accessibility Rule, three-stage connect→browse→sync flow with selective per-skill sync, manifest format = PAI's `SKILL.md` + 6 datawatch extensions (compatibility/dependencies/applicability/cost-hints/verification/MCP-tool decls), session resolution as filesystem injection (option C, BL219-aligned cleanup) + `skill_load` MCP tool (option D), Secrets-Store Rule enforced for any private-repo auth tokens. Also lands the new **Skills-Awareness Rule** in AGENT.md. See [CHANGELOG.md](CHANGELOG.md) for full history.
