...
**Current release: v6.10.0 (2026-05-05).** Minor: BL259 Phase 1 — Evals Framework. Replaces the legacy binary verifier with rubric-based grading across 4 grader types (string_match, regex_match, binary_test, llm_rubric stubbed). Suites at `~/.datawatch/evals/<name>.yaml`; runs persisted to `~/.datawatch/evals/runs/<id>.json`. Two modes: capability (~70% pass target) vs regression (~99% pass target). New `internal/evals` package + 7-surface parity. PWA Settings → Agents → Evals card lists suites with run buttons + recent run history with PASS/FAIL badges. See [CHANGELOG.md](CHANGELOG.md) for full history.
