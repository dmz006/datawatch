...
**Current release: v5.28.3 (2026-04-30).** BL214 UX fix — operator-asked: language picker promoted to **top of the datawatch identity card** (Settings → About → Language) where it belongs as a primary identity choice, not buried under General. PWA UI language is now treated as the **default app language**: changing the picker also syncs `whisper.language` (transcription input language) via PUT /api/config so transcription matches the spoken UI by default. The redundant standalone whisper.language dropdown was removed from the Whisper config card and replaced with a read-only "tracks PWA language" indicator pointing at the canonical control. Both the new About-card picker and the legacy Settings → General → Language picker stay in sync. Override path for power-users who need a different transcription language than UI language: `datawatch config set whisper.language <code>` or YAML. Carries forward v5.28.2's BL173-followup closure. **i18n wave 2:** confirm-modal `Yes`/`No` buttons (`showConfirmModal`), session dialog titles (delete/stop/restart-session via `dialog_*` Android keys), batch-delete count placeholder via `%1$d`, alerts-tab loading + empty-state, Autonomous-tab `templates` filter label + New-PRD FAB title. New universal keys (`action_yes`/`action_no`/`common_loading`/`common_no_alerts`) added to all 5 bundles + filed for upstream Compose-Multiplatform mirror at [datawatch-app#39](https://github.com/dmz006/datawatch-app/issues/39) per the v5.28.0 Localization Rule. **BL173-followup:** cluster→parent push handler verified end-to-end (`POST /api/observer/peers/{name}/stats` → aggregator) on the local daemon; new "Production-cluster reachability check" runbook in `docs/howto/federated-observer.md` with the exact pod-side curl + cleanup commands so the production-cluster verification is operator-runnable when convenient. Pre-6.0 testing window. Carries forward the v5.28.0 PWA i18n foundation (5 locale bundles, `t()` harness, Settings → General → Language picker). Carries forward: PRD card style harmonised with Sessions cards, 11 daemon-MCP gap-closure tools (memory_wal/test_embedder/wakeup, claude_models/efforts/permission_modes, rtk_version/check/update/discover, daemon_logs), PWA UI parity bundle (Running pulse + 3-dot generating indicator + 📜 scroll icon), config-driven `/api/quick_commands`, channel.js bridge memory tools, subsystem-aware `POST /api/reload?subsystem=<name>`, `session.claude_auto_accept_disclaimer`. The single canonical install guide is [`docs/howto/setup-and-install.md`](docs/howto/setup-and-install.md). Per-version detail in [CHANGELOG.md](CHANGELOG.md); cumulative release notes since v4.0.0 in [docs/plans/RELEASE-NOTES-v5.0.0.md](docs/plans/RELEASE-NOTES-v5.0.0.md).
