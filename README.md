...
**Current release: v6.10.1 (2026-05-05).** Patch: BL259 Phase 2 — Algorithm-Mode → Evals bridge. New `POST /api/algorithm/{id}/measure?suite=<name>` runs an eval suite at the Measure phase boundary and folds the verdict into the captured phase output. Same surface on MCP (`algorithm_measure`), CLI (`datawatch algorithm measure --suite`), and comm. Closes BL259. v6.10.0 shipped Phase 1 (the Evals framework + 7-surface CRUD). See [CHANGELOG.md](CHANGELOG.md) for full history.
