...
**Current release: v6.8.1 (2026-05-05).** Patch: BL257 Phase 2 — Identity Wizard. New 🤖 robot icon in the PWA header opens a 6-step modal walking the operator through each identity field; CLI `datawatch identity configure` mirrors it as an interactive prompt. Closes BL257 (Identity / Telos layer). v6.8.0 shipped Phase 1 (the layer itself + 7-surface CRUD). See [CHANGELOG.md](CHANGELOG.md) for full history.
