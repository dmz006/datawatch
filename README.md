...
**Current release: v6.8.0 (2026-05-05).** Minor: BL257 Phase 1 — operator identity / Telos layer. New `internal/identity` package + 7-surface parity (REST/MCP/CLI/comm/PWA/locale + YAML on-disk store). PWA Settings → Agents → Identity card lets the operator describe their role, north-star goals, current projects, values, current focus, and context notes; the daemon injects this into the wake-up L0 layer of every spawned session so AI work stays anchored to operator priorities. Phase 2 (interview-style init via robot-icon nav) is filed for v6.8.1. See [CHANGELOG.md](CHANGELOG.md) for full history.
