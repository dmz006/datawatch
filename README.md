...
**Current release: v6.9.0 (2026-05-05).** Minor: BL258 — Algorithm Mode (PAI 7-phase Observe→Orient→Decide→Act→Measure→Learn→Improve harness). Per-session state machine driven manually for v6.9.0 (REST/MCP/CLI/comm/PWA `advance` button). New `internal/algorithm` package + 7-surface parity. PWA Settings → Agents → Algorithm Mode card shows a phase strip per session with operator-driven Advance/Edit/Abort/Reset. Auto-detection of phase boundaries from LLM output is a follow-up. See [CHANGELOG.md](CHANGELOG.md) for full history.
