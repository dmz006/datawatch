...
**Current release: v6.7.2 (2026-05-04).** Patch: **BL247-followup** — the Observer→Monitor unification half of BL247 (originally specced but skipped when v6.5.1 shipped the rest) is now done. The standalone Observer top-level nav view is gone; its content (federated peer list, stats, config) is now a card at the bottom of Settings → Monitor. localStorage migration redirects operators stuck on the old view. Also closes BL247 properly. See [CHANGELOG.md](CHANGELOG.md) for full history.
