...
**Current release: v6.7.7 (2026-05-05).** Patch: BL261 — v6.7.6 padding-fix follow-up. Three more cards in Settings → Automata tab (Pipeline Manager, PRD Orchestrator, Skill Registries) had the same bare-container root cause as the v6.7.6 templates/aliases fix and were missed in that sweep; all three now wrapped in `padding:6px 12px` to match the standard card-content inset. See [CHANGELOG.md](CHANGELOG.md) for full history.
