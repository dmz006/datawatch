...
**Current release: v6.11.0 (2026-05-05).** Minor: BL260 — Council Mode (PAI multi-persona structured debate). 6 default personas (security-skeptic, ux-advocate, perf-hawk, simplicity-advocate, ops-realist, contrarian) editable at `~/.datawatch/council/personas/`. Debate mode (3 rounds) for serious decisions, quick mode (1 round) for fast perspective checks. New `internal/council` package + 7-surface parity. **Closes the BL257–BL260 PAI parity arc** (Identity / Telos, Algorithm Mode, Evals, Council all shipped). LLM responses are stubbed in v6.11.0; real per-persona inference is a v6.11.x follow-up. See [CHANGELOG.md](CHANGELOG.md) for full history.
