...
**Current release: v5.28.8 (2026-05-02).** Five PWA bug fixes: BL227 — terminal now refits after the session-completion "generating…" indicator clears (same `requestAnimationFrame+fit` pattern as BL211); BL223 — RTK upgrade card no longer renders raw JS as visible text (replaced `onclick+JSON.stringify` with `data-cmd`+`addEventListener`); BL224/BL225 — `orchestrator-flow.md` and `prd-phase3-phase4-flow.md` Mermaid diagrams now render in `/diagrams.html` (unquoted node labels with `]`/`<br/>` broke the parser — all affected labels quoted); BL222 — Settings → General no longer duplicates four Claude-specific fields (`skip_permissions`, `channel_enabled`, `claude_auto_accept_disclaimer`, `permission_mode`, `default_effort`) that belong exclusively in Settings → LLM → claude-code. Carries forward the v5.28.7 MCP mode visibility parity, v5.28.6 process stats panel, v5.28.5 KG tools + eBPF config normalization, and v5.28.4 quick_commands parity. Per-version detail in [CHANGELOG.md](CHANGELOG.md).
