...
**Current release: v6.7.6 (2026-05-05).** Patch: Settings tab reorganization. Tab order is now **General · Plugins · Comms · LLM · Agents · Automata · About** (Plugins moved to position 2; new **Agents** tab added between LLM and Automata). The Agents tab hosts Project Profiles, Cluster Profiles, Container Workers, and Tailscale (Configuration + Mesh Status) — moved from General. Also fixed: Session Templates and Device Aliases card content had no horizontal padding (rendered flush against the card edge); both now wrapped in `padding:6px 12px` to match the project's standard card-content inset. See [CHANGELOG.md](CHANGELOG.md) for full history.
