...
**Current release: v5.27.9 (2026-04-30).** Signal device-linking API completion (BL213, datawatch#31): `GET /api/link/qr` alias for the SSE QR-pair stream that the mobile companion expects, `GET /api/link/status` now runs `signal-cli listDevices` and returns the parsed device list (id/name/created/last_seen), and `DELETE /api/link/{deviceId}` removes a linked device via `signal-cli removeDevice` with guardrails (rejects non-numeric ids, the primary device id 1, missing `account_number` config). Plus BL212 follow-up (datawatch#29): the JS channel fallback (`internal/channel/embed/channel.js`) gains the same `memory_remember`/`memory_recall`/`memory_list`/`memory_forget`/`memory_stats` tool surface the Go bridge got in v5.27.7 — operator caught that ring-laptop / storage testing instances still hit the JS path via `~/.mcp.json` pointing at node, so they were silently missing memory tools. Pre-6.0 testing window. Carries forward: PRD card style harmonised with Sessions cards, 11 daemon-MCP gap-closure tools (memory_wal/test_embedder/wakeup, claude_models/efforts/permission_modes, rtk_version/check/update/discover, daemon_logs), PWA UI parity bundle (Running pulse + 3-dot generating indicator + 📜 scroll icon), config-driven `/api/quick_commands`, channel.js bridge memory tools, subsystem-aware `POST /api/reload?subsystem=<name>`, `session.claude_auto_accept_disclaimer`. The single canonical install guide is [`docs/howto/setup-and-install.md`](docs/howto/setup-and-install.md). Per-version detail in [CHANGELOG.md](CHANGELOG.md); cumulative release notes since v4.0.0 in [docs/plans/RELEASE-NOTES-v5.0.0.md](docs/plans/RELEASE-NOTES-v5.0.0.md).
