...
**Current release: v5.27.2 (2026-04-29).** Pre-6.0 testing window. Subsystem-aware hot-reload via `POST /api/reload?subsystem=<name>` (config / filters / memory) so operators can reconfigure without a full daemon restart, plus `session.claude_auto_accept_disclaimer` to auto-confirm claude-code's startup "trust this folder" / "Loading development channels" prompts. Both ship with full configuration parity across REST + MCP + CLI + comm channels + PWA. The single canonical install guide is now [`docs/howto/setup-and-install.md`](docs/howto/setup-and-install.md). Per-version detail in [CHANGELOG.md](CHANGELOG.md); cumulative release notes since v4.0.0 in [docs/plans/RELEASE-NOTES-v5.0.0.md](docs/plans/RELEASE-NOTES-v5.0.0.md).
