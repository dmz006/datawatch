...
**Current release: v6.7.3 (2026-05-04).** Patch: **BL247-followup correction** — v6.7.2 implemented the Observer→Monitor unification in the wrong direction (folded Observer into Settings → Monitor). v6.7.3 inverts: the **Observer** top-level nav stays; the Settings → Monitor sub-tab is dropped; its cards (System Statistics, Memory Browser/Maintenance, Schedules, Cooldown, Analytics, Audit Log, KG, Daemon Log) move into the top-level Observer view, with the **Federated Peers** card at the bottom. Settings tab bar drops from 7 to 6 tabs. localStorage migration moves operators stuck on `cs_settings_tab='monitor'` to `cs_active_view='observer'`. See [CHANGELOG.md](CHANGELOG.md) for full history.
