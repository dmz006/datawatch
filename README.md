...
**Current release: v6.7.4 (2026-05-04).** Patch: **BL247-followup hotfix** — v6.7.3 shipped the inverted direction (Observer top-level absorbs Monitor cards) but the Observer view rendered empty because `secContent()` was scoped local to `renderSettingsView` and threw a `ReferenceError` when `renderObserverView` called it. Promoted `secContent` to module-level (next to `settingsCollapsed`) so both renderers share it. v6.7.3's structural change is otherwise unchanged. See [CHANGELOG.md](CHANGELOG.md) for full history.
