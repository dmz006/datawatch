...
**Current release: v6.7.1 (2026-05-04).** Patch: BL255-followup — Skill Registries card buttons (Connect/Browse/Edit/Delete + the browse-modal Sync-selected) were no-ops because `JSON.stringify(name)` produced `"name"` with raw `"` chars that closed the inline `onclick="..."` attribute mid-string. Same bug pattern that v5.26.3 fixed for `renderPRDActions`; same fix (`escHtml(JSON.stringify(...))` so embedded `"` becomes `&quot;`). v6.7.0 minor closed BL255 Skill Registries (PAI default + 7-surface CRUD + sync flow). See [CHANGELOG.md](CHANGELOG.md) for full history.
