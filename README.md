...
**Current release: v6.7.5 (2026-05-05).** Patch: layout polish. Bottom nav buttons now spread evenly across the bar at every viewport (previously left-huddled on widths under 600px because the BL239 `space-around` rule only applied at ≥600px). **Launch Automaton wizard** + **PRD Edit Settings** modals tightened — wizard-section padding 12px→8px and section-title margin 8px→4px, settings-modal form-gap 10px→6px, footer separated by a thin border so it reads as a clear action row. See [CHANGELOG.md](CHANGELOG.md) for full history.
