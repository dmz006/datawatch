...
**Current release: v5.27.10 (2026-04-30).** MCP channel bridge introspection (BL216) — operator-flagged: "is ring-laptop using the Go bridge or the JS fallback, and how do I check?" Answer surfaced through full configuration parity: new `GET /api/channel/info` returns `{kind: "go"|"js", path, ready, hint, node_path, node_modules, stale_mcp_json: [...]}`; new `channel_info` MCP tool; new `datawatch channel info` + `datawatch channel cleanup-stale-mcp-json` CLI subcommands; new chat `channel info` command; new PWA Monitor → MCP channel bridge panel; per-session register-time daemon log line `[channel] session <id> registered with <kind> bridge at <path>`. Plus a related fix: `WriteProjectMCPConfig` (BL109) now writes the Go-bridge shape when available — previously it always wrote `Command: <node>, Args: [<channel.js>]` even when the Go bridge was active, leaving stale `.mcp.json` files pointing at non-existent JS files (the ring-laptop case). datawatch-app#38 tracks the mobile-side parity. Pre-6.0 testing window. Carries forward: PRD card style harmonised with Sessions cards, 11 daemon-MCP gap-closure tools (memory_wal/test_embedder/wakeup, claude_models/efforts/permission_modes, rtk_version/check/update/discover, daemon_logs), PWA UI parity bundle (Running pulse + 3-dot generating indicator + 📜 scroll icon), config-driven `/api/quick_commands`, channel.js bridge memory tools, subsystem-aware `POST /api/reload?subsystem=<name>`, `session.claude_auto_accept_disclaimer`. The single canonical install guide is [`docs/howto/setup-and-install.md`](docs/howto/setup-and-install.md). Per-version detail in [CHANGELOG.md](CHANGELOG.md); cumulative release notes since v4.0.0 in [docs/plans/RELEASE-NOTES-v5.0.0.md](docs/plans/RELEASE-NOTES-v5.0.0.md).
